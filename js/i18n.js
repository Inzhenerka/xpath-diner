...
var levelTranslations = {
  ru: [
    ...
    {
      doThis: 'Выберите первую тарелку с оператором =>',
      helpTitle: 'Оператор стрелки',
      help: 'XPath 3.1 позволяет передавать результат одной функции в другую с помощью оператора =>. Браузеры обычно поддерживают только XPath 1.0, поэтому этот синтаксис может не работать.',
      examples: [
        '<strong>//items/item => sort-by(@price) => head()</strong> выбирает самый дешевый элемент'
      ]
    }
  ]
};
